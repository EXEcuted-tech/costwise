...
const SystemMaintenance = () => {
  ...
  const currentListPage = filteredReleaseNotes.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  const handleViewDatabase = () => {
    const phpMyAdminUrl =
      process.env.NEXT_PUBLIC_PHPMYADMIN_URL || "http://localhost/phpmyadmin";
    ...
    <div className="w-full h-screen bg-background dark:bg-[#121212]">
      ...
      <div className="ml-[45px]">
        ...
        <div className="flex mx-16 mt-12 mb-9 gap-[5rem] justify-center">
          ...
          <div
            className={`${
              isOpen ? "w-[40%] 2xl:w-[32%]" : "w-[32%]"
            } flex flex-col p-4 h-[17rem] bg-white dark:bg-[#3C3C3C] rounded-lg drop-shadow-lg hover:animate-shrink-in2`}
          >
            <div className="flex flex-col mb-8 dark:text-white">
              <div className="flex">
                {isLoading ? (
                  <Loader className="h-12 w-[2rem]" />
                ) : (
                  <span className="text-[38px] mr-2">
                    {storage?.total.split(" ")[0]}
                  </span>
                )}
                {isLoading ? (
                  ""
            ...
            <div className="flex flex-col dark:text-white">
              <div className="w-full h-[1.5rem] bg-gray-100 rounded-full">
                <div
                  className="h-[1.5rem] bg-[#DD8383] rounded-full"
                  style={{
                    width: `${
                      (parseFloat(storage?.used.split(" ")[0]) /
                        parseFloat(storage?.total.split(" ")[0])) *
                      100
                    }%`,
                  }}
                ></div>
              ...
              {isLoading ? (
                <Loader className="mt-2 h-6 w-4" />
              ) : (
                <span className="text-[17px] pt-1 pl-1 font-light">
                  {storage?.used || "0 GB"} used
                </span>
              )}
            </div>
            <button
        ...
        <div
          className={`${
            isOpen ? "mx-9 2xl:mx-16" : "mx-16"
          } flex flex-col h-[31rem] bg-white dark:bg-[#3C3C3C] rounded-lg drop-shadow-md`}
        >
          {/* Header */}
          <div className="flex w-full h-[4rem] px-4 py-3 font-medium bg-white dark:bg-[#3C3C3C] border border-[#9290906c] drop-shadow-md rounded-t-lg">
            ...
            <span className="text-[28px] text-[#5B5353] dark:text-white">
              Release Notes
            </span>
            <button
              className={`${
            ...
            <div className="mt-[3px] ml-auto text-gray-600">
              ...
              <input
                className={`${
                  isOpen ? "w-[20rem]" : "w-[20rem]"
                } bg-white h-8 px-5 pl-9 text-[1.1em] border border-[#9290906c] dark:bg-[#3C3C3C] dark:border-[#5C5C5C] dark:text-white rounded-lg focus:outline-none`}
                type="search"
                name="search"
          ...
          {isLoading ? (
            ...
            <div
              className={`${isOpen ? "px-10" : "px-16"} w-full h-[21rem] mt-6`}
            >
              ...
              {currentListPage.length === 0 && (
                <div className="text-center  items-center justify-items-center text-[#555555] dark:text-white mt-20">
                  ...
                  <p className="text-[20px]">No release notes found.</p>
                </div>
              )}
            </div>
          )}
