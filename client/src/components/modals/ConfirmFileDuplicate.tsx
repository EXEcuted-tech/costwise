...
const ConfirmFileDuplicate: React.FC<ConfirmFileDuplicateProps> = ({ onClose, onProceed, isLoading}) => {
  useEffect(() => {
              ...
              <div className="flex justify-center">
                  <div className='flex flex-col items-center justify-center'>
                      ...
                      <div className='text-center text-[20px] text-[#9D9D9D] dark:text-[#d1d1d1] break-words'>
                          <p>You are uploading a duplicate file. Do you want to proceed?</p>
                      </div>
                      ...
                      <div className='my-[20px] px-[50px] grid grid-cols-2 gap-4'>
                          <div className="relative inline-flex bg-white border-1 border-primary overflow-hidden text-primary flex items-center justify-center rounded-[30px] cursor-pointer transition-all rounded group"
                              onClick={onProceed}>
                              <button className="flex justify-center items-center text-[19px] font-black before:ease relative h-12 w-40 overflow-hidden bg-primary text-white shadow-2xl transition-all before:absolute before:right-0 before:top-0 before:h-12 before:w-6 before:translate-x-12 before:rotate-6 before:bg-white before:opacity-10 before:duration-700 hover:shadow-primary hover:before:-translate-x-40">
                                  {isLoading && <Spinner className='size-[10px] mr-[5px]'/>}
