...
import { RiFileWarningFill, RiLockPasswordLine, RiLockPasswordFill } from "react-icons/ri";
...
const SendEmailDialog: React.FC<SendEmailDialogProps> = ({ setDialog }) => {
  ...
  const handleSubmit = async (e: { preventDefault: () => void; }) => {
    ...
    try {
      const responses = await api.post('/pass-reset/changepass', { 
          ...
          <form onSubmit={handleSubmit}>
            <div className='flex flex-col w-full px-5 gap-2'>
              ...
              <div className='relative flex justify-center items-center'>
                ...
                >
                  {showPassword3 ? <VscEyeClosed size={30} className="text-black mr-2" /> : <VscEye size={30} className="text-black mr-2" />}
                </button>
              </div>
